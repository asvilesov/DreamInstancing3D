...
class GaussianModel:
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        print("Num Split: ", len(means), self.percent_dense*scene_extent)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        print("Num Clone: ", len(new_xyz), self.percent_dense*scene_extent)
...
class CompositionalModel:
    ...
    def __init__(self ,sh_degree, no_objs, obj_paths, obj_trainers, device):
        ...
        for i in range(no_objs):
            cur_obj = obj_trainers[i].renderer.gaussians #GaussianModel(sh_degree)
            self.objects.append(cur_obj)

            if i==0: ## First object is anchor
                root_rad = self.get_radius(cur_obj)

                self.glob_trans_list.append(nn.Parameter(torch.zeros((1,3), device=self.device).unsqueeze(0).float().requires_grad_(False)))
                self.glob_rot_list.append(nn.Parameter(torch.tensor([1,0,0,0], device=self.device).unsqueeze(0).float().requires_grad_(False)))
                self.glob_scale_list.append(nn.Parameter(torch.tensor([0.8], device=self.device).unsqueeze(0).float().requires_grad_(False)))

            else: ## For second object, we provide some dummy init. values (these are updated)
                self.glob_trans_list.append(nn.Parameter(torch.tensor([0.0,0.0,0.0], device="cuda").unsqueeze(0).unsqueeze(0).float().requires_grad_(True)))
                self.glob_rot_list.append(nn.Parameter(torch.tensor([1,0,0,0], device=self.device).unsqueeze(0).float().requires_grad_(True)))
                self.glob_scale_list.append(nn.Parameter(torch.tensor([0.27], device=self.device).unsqueeze(0).float().requires_grad_(True)))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            {'params': self.glob_trans_list, 'lr': 2e-6, "name": "glob_trans"},
            {'params': self.glob_rot_list, 'lr': 2e-6, "name": "glob_rot"},
            {'params': self.glob_scale_list, 'lr': 1e-7, "name": "glob_scale"},
        ]
...
class Renderer:
    def __init__(self, config):
        ...
        self.config = config.gs3d
        self.total_config = config
        self.device = self.config.device
        torch.set_default_device(self.device) #TODO, better way of doing this?
    ...
    def sphere_pcd(self, num_pts=5000, radius=10):
    ...
    def initialize(self, input=None, num_pts=5000, radius=0.5):
        if input is None:
            ...
            gauss_init = point_e_intialize(self.total_config)
            self.gaussians.direct_init(gauss_init)
...
class RendererComposition:
    ...
    def initialize(self,obj_trainers):
        # load checkpoint
        self.gaussians = CompositionalModel(self.sh_degree, self.config.num_objs, self.config.obj_paths, obj_trainers, self.device)
